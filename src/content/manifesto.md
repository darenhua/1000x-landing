...
I'm a CS student and I hear nonstop from my peers -- their fears that they will be replaced in industry because they they never formally learned to code. They use AI to write everything. I have peers who swing the opposite direction: pretending that claude code don't even exist because they want to put in the reps themselves and learn how to code the "right way". I hear from industry, that junior engineers open slop PRs and seniors are left to suffer in code review.

1000x dev is the tool that takes you from being a {% footnote note="By earning your own engineering insight. \n By being exposed to problems and the paths to solve them. \nBy being in the loop about the 'how'" %}programmer to being an engineer{% /footnote %}. While reaping the 1000x velocity rewards of AI that the best engineers are enjoying today.
...
- You're probably missing many assumptions

Even if you do, you probably don't understand how to decompose it.
- You're probably not working at the right level of abstraction

Even if you do, you probably don't understand how to solve these subproblems. 
- You need to understand the "process", the "sequence of decisions", the "how"

Then, what if you had a folder full of code examples, each serving as working solutions to each subproblem. And you made sure that you understood the logic of each code example before you start integrating them into your codebase? {% footnote note="give me your email at waitlist, and I can show you what workflow I want" %}That's the workflow I want.{% /footnote %}
...
Over time, you build something like an {% footnote note="credit: oliver keh." %}engineering journal{% /footnote %}. A repository full of code insights. Some are global, like button UIs, form validations, email sending, auth, that recur across projects. Some are scoped to a project, like a users data model code example for your LinkedIn clone.
...
Prompting back and forth is bad progress-time efficiency. A project is probably thousands of followup-prompts from completion. You can't parallelize follow-up prompts well. Each prompt probably takes 5 minutes. Boilerplate prompts are even worse. 

Verifying a live preview is very high efficiency. A project probably is probably 20 well-designed nested subproblems away from completion. You can make prototypes and variants in parallel. Verifying takes less than 30 seconds and my favorite benefit: *it's so fun*.
